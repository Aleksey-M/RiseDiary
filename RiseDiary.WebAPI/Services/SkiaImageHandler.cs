...
public abstract class SkiaImageHandler
{
    ...
    protected static byte[] CropImage(byte[] data, int left, int top, int width, int height, int imageQuality)
    {
    ...
    protected static (int width, int height) GetImageSize(byte[] data)
    {
        using var bitmap = SKBitmap.Decode(data);
        return (bitmap.Width, bitmap.Height);
    }

    protected static (DateTime? taken, string? cameraModel) GetMetadataFromPhoto(byte[] image)
    {
        ImageFile file;
        ...
        DateTime? taken = prop != null ? (DateTime)prop.Value : null;
    ...
    protected static byte[] RotateImage(byte[] image, float degrees, int imageQuality)
    {
        ...
        using (var surface = new SKCanvas(rotated))
        {
            ...
            surface.DrawBitmap(bitmap, 0, 0, new SKSamplingOptions(SKFilterMode.Linear));
        }
