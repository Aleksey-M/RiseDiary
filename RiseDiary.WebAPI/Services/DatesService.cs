...
public sealed class DatesService : IDatesService
{
    ...
    public DatesService(DiaryDbContext context, ISettingsService appSettingsService)
    {
    ...
    {
        var scopeId = (await _appSettingsService.GetSetting<ImportantDaysSettings>(cancellationToken)).Data?.ImportantDaysScopeId
            ?? throw new Exception("Setting 'ImportantDaysScopeId' does not exists");
    ...
    {
        var daysCount = (await _appSettingsService.GetSetting<ImportantDaysSettings>(cancellationToken)).Data?.ImportantDaysDisplayRange
