...
public sealed class RecordsImagesService : IRecordsImagesService
{
    ...
    public async Task<RecordImageEntity> AddRecordImage(Guid recordId, Guid imageId, int? order = null)
    {
        ...
        recordImage ??= new RecordImageEntity
        {
            ImageId = imageId,
            RecordId = recordId,
            Order = 0
    ...
    public async Task<List<RecordImageEntity>> ChangeRecordImageOrder(Guid recordId, Guid imageId, int newOrder)
    {
        var list = await _context.RecordImages
               .Include(x => x.Image)
               .Where(x => x.RecordId == recordId)
    ...
    public async Task<List<RecordImageEntity>> GetLinkedImagesList(Guid recordId, CancellationToken cancellationToken) =>
